...
void team_conv_sparse(float ** * image, struct sparse_matrix ** * kernels,
   ...
   #pragma omp parallel for if (nkernels > 400) private(w, h, m, x, y, kernelSize, imgSize, kernel_order, nkernels, end, value, outputSaver, imageRef, kernel) shared(image, kernels, output) collapse(2) 
