...
void team_conv_sparse(float ** * image, struct sparse_matrix ** * kernels,
   ...
   #pragma omp parallel for if (nkernels > 63) private(imgSize,kernelSize, width, height, kernel_order, kernel_order, nkernels, end, channel, value, outputSaver) shared(image, kernels, output) collapse(3)
